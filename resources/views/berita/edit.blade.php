                    ...
                    <form action="{{ route('berita.update', $berita->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <div class="card-title">Ubah @yield('title')</div>
                            </div>
                            <div class="card-body">
                                <div class="row mb-4">
                                    <label class="col-md-3 form-label">Judul Berita:</label>
                                    <div class="">
                                        <input type="text" name="judul_berita" class="form-control @error('judul_berita') is-invalid state-invalid @enderror" value="{{ $berita->judul_berita }}">
                                        @error('judul_berita')
                                    ...
                                    <div class="">
                                        <select name="kategori" class="form-control form-select select2 @error('kategori') is-invalid state-invalid @enderror" data-bs-placeholder="Pilih Kategori">
                                            ...
                                            @foreach ($kategori as $item)
                                                <option value="{{ $item->id }}" {{ $item->id == $berita->idkategori ? 'selected' : ''  }}>{{ $item->namakategori }}</option>
                                            @endforeach
                                        </select>
                                        @error('kategori')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                ...
                                <div class="row">
                                    ...
                                    <div class="mb-4">
                                        <textarea class="content" name="berita" required>{{ $berita->berita }}</textarea>
                                    </div>
                                </div>
                                <!--End Row-->
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="file" class="form-control-file @error('picture') is-invalid state-invalid @enderror" id="picture" name="gambar" value="{{ $berita->gambar }}">
                                    @error('picture')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
